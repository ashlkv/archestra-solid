...
class ZhipuaiChatCompletionInteraction implements InteractionUtils {
    ...
    constructor(interaction: Interaction) {
        this.request = interaction.request as archestraApiTypes.ZhipuaiChatCompletionRequest;
        this.response = interaction.response as archestraApiTypes.ZhipuaiChatCompletionResponse;
    ...
    getToolRefusedCount(): number {
        let count = 0;
        for (const choice of this.response.choices) {
            if (choice.finish_reason === "sensitive" || choice.finish_reason === "network_error") {
                count++;
            }
        }
        return count;
            ...
            if (msg.role === "assistant" && "tool_calls" in msg && msg.tool_calls) {
                ...
                for (const toolCall of msg.tool_calls) {
                    const toolResultMsg = messages
                        .slice(i + 1)
                        .find((m) => m.role === "tool" && "tool_call_id" in m && m.tool_call_id === toolCall.id);
    ...
    private mapResponseToUiMessages(): PartialUIMessage[] {
        return this.response.choices.map((choice) => this.mapToUiMessage(choice.message));
    }
