...
class OpenAiChatCompletionInteraction implements InteractionUtils {
    ...
    constructor(interaction: Interaction) {
        this.request = interaction.request as archestraApiTypes.OpenAiChatCompletionRequest;
        this.response = interaction.response as archestraApiTypes.OpenAiChatCompletionResponse;
        ...
        for (const message of this.request.messages) {
            if (message.role === "assistant") {
                ...
                if (refusal && refusal.length > 0) {
                    const toolName = refusal.match(/<archestra-tool-name>(.*?)<\/archestra-tool-name>/)?.[1];
                    if (toolName) {
