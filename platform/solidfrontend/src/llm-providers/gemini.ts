...
function hasFunctionResponse(part: unknown): part is GeminiFunctionResponsePart {
    return (
        typeof part === "object" && part !== null && "functionResponse" in part && part.functionResponse !== undefined
    );
}
...
function hasText(part: unknown): part is GeminiTextPart {
    return (
        typeof part === "object" && part !== null && "text" in part && typeof (part as GeminiTextPart).text === "string"
    );
}

function hasInlineData(part: unknown): part is GeminiInlineDataPart {
    return typeof part === "object" && part !== null && "inlineData" in part && part.inlineData !== undefined;
}

function hasFileData(part: unknown): part is GeminiFileDataPart {
    return typeof part === "object" && part !== null && "fileData" in part && part.fileData !== undefined;
}
...
class GeminiGenerateContentInteraction implements InteractionUtils {
    ...
    constructor(interaction: Interaction) {
        this.request = interaction.request as archestraApiTypes.GeminiGenerateContentRequest;
        this.response = interaction.response as archestraApiTypes.GeminiGenerateContentResponse;
            ...
            if (message.role === "model" && Array.isArray(message.parts)) {
                for (const part of message.parts) {
                    if (hasText(part) && part.text) {
                        const toolName = part.text.match(/<archestra-tool-name>(.*?)<\/archestra-tool-name>/)?.[1];
                        if (toolName) {
        ...
        for (const message of reversedMessages) {
            ...
            if (Array.isArray(message.parts)) {
                ...
                for (const part of message.parts) {
                    ...
                    if (hasFileData(part)) {
                        const fileName = part.fileData.fileUri.split("/").pop() || part.fileData.fileUri;
    ...
    private mapToUiMessage(
        ...
        const { role } = content;
        const contentParts = "parts" in content ? content.parts : content.content?.parts;
    ...
    private mapRequestToUiMessages(dualLlmResults?: DualLlmResult[]): PartialUIMessage[] {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (msg.role === "user" && Array.isArray(msg.parts)) {
                const hasOnlyFunctionResponses = msg.parts.every((part) => hasFunctionResponse(part));
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (msg.role === "model" && Array.isArray(msg.parts)) {
                const hasFunctionCallPart = msg.parts.some((part) => hasFunctionCall(part));
                ...
                if (hasFunctionCallPart) {
                    ...
                    for (const part of msg.parts) {
                        if (hasFunctionCall(part) && part.functionCall) {
                            ...
                            if (functionResponseMsg && Array.isArray(functionResponseMsg.parts)) {
                                ...
                                if (functionResponsePart && hasFunctionResponse(functionResponsePart)) {
                                    const output = functionResponsePart.functionResponse.response || {};
    ...
    mapToUiMessages(dualLlmResults?: DualLlmResult[]): PartialUIMessage[] {
        return [...this.mapRequestToUiMessages(dualLlmResults), ...this.mapResponseToUiMessages()];
    }
}
