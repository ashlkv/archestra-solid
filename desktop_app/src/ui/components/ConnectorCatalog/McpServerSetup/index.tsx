...
export default function McpServerSetup({ open, onOpenChange, provider, content, status }: { open: boolean, onOpenChange: (open: boolean) => void, content: string, provider: string, status?: string }) {
  const handleClose = () => {
    onOpenChange(false);
  };
